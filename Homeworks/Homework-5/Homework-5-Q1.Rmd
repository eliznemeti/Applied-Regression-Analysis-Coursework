...
Full model: E[Job_Satisfaction] = ß0 + ß1(Work_Hours) + ß2(Job_Role) + ß3(Work_Hours*Job_Role)
...
We need to do 3 steps: get predicted scores (aka point estimates) from the model for an office worker who works 40 hours per week, and a manual laborer who works 35 hours per week. Then, we need to find the SE of the difference between then. Finally, we can find the CI for the difference in predictions. 

prediction = ß0 + (Hours Worked * Work Hours Effect for Manual Laborers) 
...
prediction = ß0 + Job Role Effect for Office Worker + (Hours Worked*Work Hours Effect for Office Workers) + (Hours Worked*Interaction Term)
prediction = ß0 + ß2 + (40*ß1) + (40*ß3)
...
```{r}
# SE for Manual Laborer for 35 hours
n <- nrow(ABCD_data)
mean_hours <- mean(ABCD_data$Work_Hours)
SS <- sum((ABCD_data$Work_Hours - mean_hours)^2)

residual_se <- summary(full_model)$sigma

SE_manual_laborer <- residual_se * sqrt((1/n) + ((35 - mean_hours)^2 / SS))
SE_manual_laborer
