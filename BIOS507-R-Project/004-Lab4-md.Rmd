...
Yes. Now we are ready to start fitting our model. 

***
3. Fitting the Model

We can start by writing down the assumed regression model.
...
***
4. Accepting/Rejecting the Null Hypothesis
...
***
5. Obtaining the P-value and confidence intervals

Next, I want to obtain the corresponding p-value for this test.
This is a way of extracting the point estimate and the standard error without requiring us to write type them in manually.
...
test_statistic = (point_estimate - 0) / se_hat # Calculate the test statistic

pt(test_statistic, df = nrow(cc_data) - 2, lower.tail = FALSE) * 2 # Multiply by 2 because doing a two-sided test
```
